...
contract FlightSuretyApp {
    ...
    event AirlineVoting(address airline, uint256 votes);
    ...
    */
    ...
    modifier requireIsActive(){
        require(flightSuretyData.isActive(msg.sender), "Caller is not funded so not Active");
        _;
    }

    modifier requireNotVoted(address airlineAddress) {
        require(!checkVotersList(voters[airlineAddress]), "You already voted to register this airline.");
        _;
    }
    ...
    */   
    ...
    {
        if(flightSuretyData.getAirLineCounter() < MIN_AIRLINES) {
            flightSuretyData.registerAirline(airlineAddress, name);
        } else {
            flightSuretyData.savePendingAirline(airlineAddress, name);
        }
    }
                            ...
                            requireNotVoted(airlineAddress)
    {
        ...
        uint256 votes = flightSuretyData.vote(airlineAddress);
        emit AirlineVoting(airlineAddress, votes);
    }
    ...
    */  
                                ...
                                requireIsOperational
    {
        ...
        emit ProcessedFlightStatus(key, statusCode);
    }
...
// endregion
    ...
    function getCreditToPay
                                ...
                                returns (uint256) 
    {
        return flightSuretyData.getCreditToPay(msg.sender);
    }
...
contract FlightSuretyData {
    ...
    function isActive(address airlineAddress) external view returns(bool);

    function isAirlineRegistered(address airlineAddress) public view returns(bool);

    function isAirlinePending(address airlineAddress) external view returns(bool);

    function getAirLineFund(address airlineAddress) external view returns(uint256);

    function getPassengerCredits(address passengerAddress) external view returns(uint256);

    function getCreditToPay(address passengerAddress) external view returns(uint256);
    ...
    function savePendingAirline(address airlineAddress, string name) external;

    function vote(address airlineAddress) external returns (uint256);
